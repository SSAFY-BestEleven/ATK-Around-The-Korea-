...
public class PlanServiceImpl implements PlanService {
	...
	public void modifyOrder(List<TravelPlanOrderRequest> orderList, Integer planId) {
		...
		Map<Integer, Integer> orderMap = orderList.stream().collect(
				Collectors.toMap(TravelPlanOrderRequest::getAttractionInfoId, TravelPlanOrderRequest::getOrderIndex));
	...
	@Override
	public void deleteByTravelPlanId(Integer planId, Integer travelPlanId) {
		int orderIndex = travelPlanRepository.getReferenceById(travelPlanId).getOrderIndex();
		List<TravelPlan> travelPlans = travelPlanRepository.findAllByPlanId(planId);
		travelPlanRepository.deleteById(travelPlanId);
		for (TravelPlan travelPlan : travelPlans) {
			if (travelPlan.getOrderIndex() > orderIndex) {
				System.out.println(travelPlan);
				travelPlan.setOrderIndex(travelPlan.getOrderIndex() - 1);
				travelPlanRepository.save(travelPlan);
			}
		}
	}
